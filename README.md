...
# InfluxDB
`docker create --volume=/data --name influxdb_data busybox && docker run -d -p 8083:8083 -p 8086:8086 --env-file etc/docker-env --name influxdb --volumes-from influxdb_data dschien/influxdb:latest`

# Paul's Dev Environment

# Quick Start for once Set up

1. Open Kitematic
2. Open PowerShell
3. Change WD to ep_site root on c:
4. Configure Docker - `docker-machine env default --shell powershell  | Invoke-Expression` - check with `docker ps`
5. Run DB container `docker run -p 5432:5432 --name db --env-file etc/env -d --volumes-from pg_data postgres:9.4`
6. Run web-server: `docker run --rm -ti -p 8000:8000 -p 8888:8888 -P --link db:db -v /c/Users/edxps/PycharmProjects/ep_site:/ep_site -w /ep_site web python manage.py runserver --settings=ep_site.dev_settings`
7. Open (e.g. via Kitematic)


1. Paul Activate in normal windows command! : `activate ep`
2. PowerShell - `docker-machine start default`
3. `docker-machine env default --shell powershell  | Invoke-Expression`
4. `docker restart db`
5. run server in Pycharm - top right